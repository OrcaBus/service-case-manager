...
class ModelTestCase(TestCase):
    ...
    def test_get_simple_model(self):
        ...
        # user
        self.assertEqual(case.user_set.all().count(), 1, 'correct number of user linked to case')
        ...
        self.assertEqual(case_user_link.description, 'lead', 'correct case user link description assigned')

        # external entity
        self.assertEqual(case.external_entity_set.all().count(), 2, 'correct number of external entity linked to case')
        external_entity_one = case.external_entity_set.get(alias='library-001')
        self.assertEqual(external_entity_one.alias, 'library-001', 'correct external entity alias assigned')
        external_entity_two = case.external_entity_set.get(alias='library-002')
        self.assertEqual(external_entity_two.alias, 'library-002', 'correct external entity alias assigned')
        ...
        self.assertEqual(comments.count(), 1, 'correct number of comment linked to case')
